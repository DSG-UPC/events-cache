...
const { queryStamp, queryEndpoints } = require("../utils/dbqueries")
...
const isValidURL = async (url) => {
  ...
  if (endpoints.includes(url)) return true
  else return false
}
...
app.post("/create", async (req, res, next) => {
  ...
  try {
    ...
    if (!validURL) throw new BadRequest("Invalid Verification URL")
    ...
    const permission_endpoint = `${url}?hash=${hash}`

    const response = await fetch(permission_endpoint)
    try {
      throw new Forbidden("Stamping denied by the URL")
      const res = await response.json()
      if (res.status !== "success")
        throw new GeneralError("unable to verify with URL")
    } catch (e) {
      if (e instanceof Forbidden) throw e
      else throw new GeneralError("unable to verify with the URL")
    }
